...
def cal_capital_income(interest_income, capital_income):
    """
...
@iterate_jit(nopython=True)
def cal_total_income(emp_income, interest_income, business_income, total_income):
    """
    Compute total gross income.
    """
    total_income = emp_income + interest_income + business_income
    return total_income
...
@iterate_jit(nopython=True)
def cal_mortgage_deduction(mortgage_deduction_threshold,
                           mortage_interest,
                           home_own_saving_plan_dep,
                           mortgage_deduction):
    """
    Compute mortgage deduction.
    """
    mortgage_deduction = min(
        mortgage_deduction_threshold,
        max(mortage_interest, home_own_saving_plan_dep)
    )
    return mortgage_deduction


@iterate_jit(nopython=True)
def cal_pension_deduction(pension_deduction_threshold,
                          pension_contribution,
                          pension_deduction):
    """
    Compute pension deduction.
    """
...
@iterate_jit(nopython=True)
def cal_ahl_deduction(AHL_deduction_rate, total_income, AHL_deduction):
    """
    Compute Affordable Housing Levy deduction.
    """
...
@iterate_jit(nopython=True)
def cal_total_deductions(mortgage_deduction,
                         pension_deduction,
...
@iterate_jit(nopython=True)
def cal_disability_exemption(disability_exemption_threshold,
                             is_disabled,
                             disability_exemption):
    """
    Compute disability exemption.
    """
    if is_disabled == 1:
...
@iterate_jit(nopython=True)
def cal_net_taxable_income(disability_exemption,
                           emp_income_total,
                           business_income,
                           total_deductions,
                           net_taxable_income):
    ...
    net_taxable_income = max(
        0.0,
        emp_income_total + business_income - total_deductions - disability_exemption
    )
...
def cal_ti_behavior(rate1, rate2, rate3, rate4, rate5,
    ...
    frac_change_net_of_pit_rate = (
        ((1.0 - marg_rate) - (1.0 - marg_rate_curr_law))
        / (1.0 - marg_rate_curr_law)
    )

    frac_change_income_wage = elasticity * frac_change_net_of_pit_rate
    income_wage_behavior = income_wage_l * (1.0 + frac_change_income_wage)

    return income_wage_behavior


@iterate_jit(nopython=True)
def cal_pit_w(rate1, rate2, rate3, rate4, rate5,
              tbrk1, tbrk2, tbrk3, tbrk4, tbrk5,
              income_wage_behavior, pitax_w):
    """
    Compute PIT on wage/business income using a 5-bracket system.
    """
    inc = income_wage_behavior

    pitax_w = (
        rate1 * min(inc, tbrk1) +
        rate2 * min(tbrk2 - tbrk1, max(0.0, inc - tbrk1)) +
        rate3 * min(tbrk3 - tbrk2, max(0.0, inc - tbrk2)) +
        rate4 * min(tbrk4 - tbrk3, max(0.0, inc - tbrk3)) +
        rate5 * min(tbrk5 - tbrk4, max(0.0, inc - tbrk4))
    )
...
@iterate_jit(nopython=True)
def cal_reliefs(personal_relief,
                insurance_relief_threshold,
                insurance_relief,
                total_reliefs):
    """
    Compute total reliefs.
    """
...
def cal_total_pit(pitax_w, pitax_c, total_reliefs, pitax):
    """
    Compute total PIT payable.
    """
    pitax = max(0.0, pitax_w + pitax_c - total_reliefs)
    return pitax